...
def main():
    ...
    with col1:
        ...
        # --- NEW FEATURES ---
        st.subheader("⚙️ Streaming Mode & Audio")
        streaming_mode = st.radio(
            "Select Mode",
            ["Normal Loop", "Audio Duration Mode (15s delay, stop at audio end)"],
            help="Normal: Loop both video and audio. Audio Mode: Video loops, audio starts after 15s, stops when audio ends."
        )
        
        uploaded_audio = st.file_uploader("🎵 Upload Audio (for Audio Duration Mode)", type=['mp3', 'wav', 'aac', 'm4a'])
        audio_path = None
        if uploaded_audio:
            audio_path = f"temp_audio_{st.session_state['session_id']}.mp3"
